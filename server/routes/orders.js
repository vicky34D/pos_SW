...
router.get('/', (req, res) => {
  ...
  if (date) {
    ...
    sql += " WHERE date(created_at) BETWEEN date(?) AND date(?)";
    params.push(from, to);
  }
...
router.post('/', (req, res) => {
  ...
  const insertOrder = db.prepare(`
    INSERT INTO orders (id, order_number, customer_name, order_type, payment_method, subtotal, tax, total)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const updateCounter = db.prepare('UPDATE order_counter SET last_number = ? WHERE id = 1');

  // Transaction for atomicity
  const createOrder = db.transaction(() => {
    insertOrder.run(orderId, nextNumber, customer_name || 'Walk-in', order_type || 'dine-in', payment_method, subtotal, tax, total);

    for (const item of items) {
      insertItem.run(orderId, item.id, item.name, item.price, item.qty);
    }

    updateCounter.run(nextNumber);
  });
